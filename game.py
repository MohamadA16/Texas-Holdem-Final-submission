...
class Game:
    ...
    def show_results(self):
        ...
        if self.player2.folded:
            print(f"\n{self.player2.name} folded.")
            print(f"Winner: {self.player1.name}")
            print(f"{self.player1.name} wins {self.pot} chips.")
...
def main(player1_name, player2_name):
     ...
     p1_odds, p2_odds, tie_odds = estimate_odds(game.player1.hand, game.player2.hand, game.community_cards)
     print(f"Odds after turn: {player1_name} {p1_odds:.1f}% | {player2_name} {p2_odds:.1f}% | Tie {tie_odds:.1f}%")
     game.betting_round()
     if game.player1.folded or game.player2.folded:
        game.show_results()
        return
